...
export async function scrapeCultures() {
  ...
  try {
    ...
    const basicEvents: ScrapedEvent[] = await page.evaluate(() => {
      const results: ScrapedEvent[] = [];

      // Pour chaque bloc .em-event.em-item
      document
        .querySelectorAll<HTMLElement>(".em-event.em-item")
        .forEach((card) => {
          // 4.1 Thumbnail
          const thumbEl =
    ...
    if (basicEvents.length === 0) {
      console.warn(
        "⚠️ Aucun événement extrait de la liste Cultures&P. ? Vérifiez les sélecteurs."
      );
    }
    ...
    for (const ev of basicEvents) {
      ...
      try {
        ...
        console.warn(
          `⚠️ Impossible de parser la date début pour "${ev.title}" : "${ev.rawDate}"`
        );
        continue;
      }
      ...
      try {
        ...
        try {
          const imgURL = await detailPage.$eval(
            ".em-item-image img",
          ...
          console.warn(
            `ℹ️ Pas d'image détail pour "${ev.title}", on gardera le thumbnail.`
          );
        }
        ...
        console.warn(
          `⚠️ Impossible d’ouvrir détail pour "${ev.title}" :`,
          innerErr
        );
      }
    ...
    console.log(
      `✅ culturesetpatrimoines : ${basicEvents.length} événements traités`
    );
  } catch (error) {
