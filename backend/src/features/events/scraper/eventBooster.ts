...
export async function scrapeEventBooster() {
  ...
  try {
    ...
    const basicEvents: ScrapedEvent[] = await page.evaluate(() => {
      ...
      const scripts = document.querySelectorAll<HTMLScriptElement>(
        ".mec-event-list-standard script[type='application/ld+json']"
      );

      scripts.forEach((script) => {
        try {
          const data: RawJSONLD = JSON.parse(script.textContent || "{}");
    ...
    for (const ev of basicEvents) {
      ...
      let detailTime: string | null = null;
      let detailLocation: string | null = null;
      let detailCategory: string | null = null;
      let detailPrice: string | null = null;
      let detailPriceCurrency: string | null = null;
      let detailType: string = "sur-place"; // par défaut “sur-place” (on ajustera à “en-ligne” si pas de lieu)
      ...
      try {
        ...
        const detailData = await detailPage.evaluate(() => {
          ...
          const timeEl = document.querySelector<HTMLAnchorElement>(
          ...
          const locationEl = document.querySelector<HTMLDivElement>(
            ".mec-single-event-location dl dd.author.fn.org"
          );
          ...
          const catNodes = Array.from(
            document.querySelectorAll<HTMLAnchorElement>(
              ".mec-single-event-category dl dd.mec-events-event-categories a"
            )
          );
        ...
        console.warn(
          `⚠️ Impossible d’extraire détail pour "${ev.title}":`,
          innerErr
        );
      }
