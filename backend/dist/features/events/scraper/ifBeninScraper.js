...
exports.scrapeIFBenin = scrapeIFBenin;
const puppeteer_1 = __importDefault(require("puppeteer"));
const db_1 = require("../../../config/db");
async function scrapeIFBenin() {
    ...
    try {
        ...
        try {
            ...
            console.warn("⚠️ .event-item non trouvé après 15 s — on continue quand même");
        }
        ...
        const scraped = await page.evaluate(() => {
            ...
            document.querySelectorAll(".events-list .event-item").forEach((el) => {
                ...
                // catégorie
                const category = el.querySelector(".ev-category span")?.textContent?.trim() || "";
                // date (ex. "05/06/2025")
                const date = el.querySelector(".ev-date span")?.textContent?.trim() || "";
                // heure (ex. "18h00")
                const time = el.querySelector(".ev-hour span")?.textContent?.trim() || "";
                // lien vers la page détail
                const link = el.querySelector(".ev-link a")?.getAttribute("href") || undefined;
                if (title && date && time) {
                    events.push({
                        title,
                        category,
        ...
        for (const e of scraped) {
            ...
            const isoString = `${yearStr.padStart(4, "0")}-${monthStr.padStart(2, "0")}-${dayStr.padStart(2, "0")}T${hourStr.padStart(2, "0")}:${minuteStr.padStart(2, "0")}:00`;
            const parsedDate = new Date(isoString);
            if (isNaN(parsedDate.getTime())) {
                console.warn(`Impossible de parser la date pour "${e.title}" : ${isoString}`);
                continue;
            }
            ...
            if (e.link) {
                try {
                    ...
                    try {
                        ...
                        fullImage = await detailPage.$eval(".featured-image img", (img) => img.getAttribute("src") || undefined);
                    }
                    catch {
                        console.warn(`ℹ️ Pas d'image grand format pour "${e.title}", on gardera la miniature`);
                    }
                    await detailPage.close();
                }
                catch (err) {
                    console.warn(`⚠️ Impossible d’ouvrir la page détail pour "${e.title}" :`, err);
                }
            }
